...
export const signup= async (req,res,next)=>{
    ...
    const html =`<a href="${link}"><!DOCTYPE html>
    <html>
    <head>
    ...
    </html></a> <br/> <br/> <br/> <a href="${RLink}"> send new email</a>`
...
export const confirmEmail = async(req,res)=>{
    ...
    if(user.modifiedCount){
        ...
        return next(new Error('Not registered account or email is verify',{cause:400}));
    }
} 
...
export const newConfirmEmail = asyncHandler(async(req,res,next)=>{
    ...
    return res.status(200).send(`<p> new confirm email sent to your inbox </p>`)
...
export const sendCode= asyncHandler(async(req,res,next)=>{
    ...
    let code = customAlphabet('123456abcdefghijklmnopqrstuvwxyzANMDJK',4);
    code =code();
...
export const forgetPassword = asyncHandler(async(req,res,next)=>{
    ...
    if(user.forgetCode!=code || !code){
        return next(new Error('invlaid code',{cause:400}))
}
